...
- Trace actual prompts and raw responses: add `--trace-prompts <directory>` to `learn`, `use`, or `ask`
...
Use `--trace-prompts <directory>` only when the user wants a record of real model exchanges. It preserves each exact prompt and raw Pi response as a user-only JSON file, including separate learning repair attempts. These files may contain the user's intent and complete CLI help evidence; treat the directory as sensitive. Do not combine tracing with `--show-prompt`.
