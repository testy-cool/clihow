...
test("delegated JSON asks stream progress on stderr while keeping stdout clean", async () => {
  const { runCli } = await import("../src/cli.ts");
  const { listThreads } = await import("../src/threads.ts");
  const root = await mkdtemp(join(tmpdir(), "clihow-question-json-stream-"));
  const binaryPath = join(root, "progress-question.mjs");
  const stdout: string[] = [];
  const stderr: string[] = [];
  const events: string[] = [];
  try {
    ...
    assert.equal(
      await runCli(["ask", "demo", "live question", "--json"], {
        env: { ...process.env, CLIHOW_HOME: root },
        stdout: (value: string) => {
          stdout.push(value);
          events.push(`stdout:${value}`);
    ...
    assert.equal(events[0], "stderr:PROGRESS:missing\n");
    assert.equal(stderr.join(""), "PROGRESS:missing\n");
    const result = JSON.parse(stdout.join(""));
    assert.equal(result.answer, "FINAL:live question");
    assert.equal(result.invocation.stderr, "PROGRESS:missing\n");
    const thread = (await listThreads(root))[0];
    assert.equal(thread?.turns[1]?.text, "FINAL:live question");
    assert.doesNotMatch(JSON.stringify(thread), /PROGRESS:/);
  } finally {
